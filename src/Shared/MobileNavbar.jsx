...
const MobileNavbar = ({active}) => {
    ...
    return (
        <>
            <nav
                className={`items-center flex lg:hidden justify-between z-50 px-6 py-5 sticky top-0 bg-white transition-shadow ${shadow ? 'shadow-lg' : ''}`}>
            ...
            <aside
                className={`${isSidebarOpen ? 'translate-x-0 opacity-100 z-[999]' : 'translate-x-[300px] opacity-0 z-[-1]'} w-[80%] md:w-[50%] h-screen overflow-y-auto bg-white p-[20px] fixed top-0 right-0 transition-all duration-300`}>
